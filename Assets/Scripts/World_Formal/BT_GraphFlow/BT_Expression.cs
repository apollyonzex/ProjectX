    ...
    public class BT_Expression : Expression<BT_Expression>
    {
        public int do_calc_int<T>(T ctx) where T : BT_Context
        {
            ...
            if (ees != null)
            {
                ...
                {
                    var ee = ees[i] as IBT_EE;

                    ee.get_value(ctx, out int value);
                    values[i] = value;
                }
        ...
        public float do_calc_float<T>(T ctx) where T : BT_Context
        {
            ...
            if (ees != null)
            {
                ...
                for (int i = 0; i < length; i++)
                {
                    ...
                    ee.get_value(ctx, out float value);
                    values[i] = value;
                }
        ...
        public bool do_calc_bool<T>(T ctx) where T : BT_Context
        {
            ...
            if (ees != null)
            {
                ...
                for (int i = 0; i < length; i++)
                {
                    ...
                    var ee = item as IBT_EE;

                    if (ee.get_value(ctx, out float v1))
                    {
                        values[i] = (v1, typeof(float));
                        continue;
                    }

                    if (ee.get_value(ctx, out int v2))
                    {
                        values[i] = (v2, typeof(int));
                        continue;
                    }

                    if (ee.get_value(ctx, out bool v3))
                    {
