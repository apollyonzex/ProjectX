...
namespace DialogFlow.Editor.NodeEditors {
    ...
    [NodeEditor(typeof(Nodes.PageOptions))]
    public class PageOptionsEditor : DialogNodeEditor {

        public new Nodes.PageOptions node => m_node as Nodes.PageOptions;
        ...
        public override void attach(GraphEditor graph, Node node) {
            ...
            this.graph = graph as DialogGraphEditor;
        ...
        protected override void clone_to(Node node) {
            base.clone_to(node);
            if (node is Nodes.PageOptions other) {
                var this_node = this.node;
                other.options = new List<PageOption>(this_node.options.Count);
                foreach (var option in this_node.options) {
        ...
        private class AddOption : GraphUndo.ICommand {
            public PageOptionsEditor editor;
